...
def derive_creator_vault(creator: Pubkey, quote_mint: Pubkey) -> tuple[Pubkey, Pubkey]:
    ...
    vault_ata = get_associated_token_address(vault_auth, quote_mint, TOKEN_2022_PROGRAM_PUB)
...
class PumpSwap:
    ...
    async def _token_program_for_mint(self, mint: Pubkey) -> Pubkey:
        # mint account owner is the token program id
        info = await self.async_client.get_account_info(mint, commitment=Processed)
        if not info.value:
            raise Exception(f"Failed to fetch mint account info for {mint}")
    ...
    async def buy(
        ...
        wsol_ata = get_associated_token_address(user_pubkey, token_quote, TOKEN_2022_PROGRAM_PUB)
        ...
        if pool_type == NEW_POOL_TYPE:
            buy_ix = self._build_new_pumpswap_buy_ix(
                ...
                user_base_token_ata  = get_associated_token_address(user_pubkey, token_base, TOKEN_2022_PROGRAM_PUB),
                user_quote_token_ata = get_associated_token_address(user_pubkey, token_quote, TOKEN_2022_PROGRAM_PUB),
            ...
            buy_ix = self._build_old_pumpswap_buy_ix(
                ...
                pool_quote_token_account = Pubkey.from_string(pool_data['pool_quote_token_account']),
                protocol_fee_recipient   = PROTOCOL_FEE_RECIP,
                protocol_fee_recipient_ata = get_associated_token_address(PROTOCOL_FEE_RECIP, token_quote, TOKEN_2022_PROGRAM_PUB),
                base_amount_out = base_amount_out,
                max_quote_amount_in = max_quote_amount_in
            )
    ...
    async def reversed_buy(
        ...
        wsol_ata = get_associated_token_address(user_pubkey, token_base, TOKEN_2022_PROGRAM_PUB)
    ...
    def _build_new_pumpswap_sell_ix(
        ...
        accs = [
            ...
            AccountMeta(pubkey=SPubkey.from_string(str(get_associated_token_address(user_pubkey, pool_data["token_base"], TOKEN_2022_PROGRAM_PUB))),
                        is_signer=False, is_writable=True),
            AccountMeta(pubkey=SPubkey.from_string(str(get_associated_token_address(user_pubkey, pool_data["token_quote"], TOKEN_2022_PROGRAM_PUB))),
                        is_signer=False, is_writable=True),
    ...
    async def create_pool(
        ...
        pool_base_ata  = get_associated_token_address(pool_pda,  base_mint, TOKEN_2022_PROGRAM_PUB)
        pool_quote_ata = get_associated_token_address(pool_pda,  quote_mint, TOKEN_2022_PROGRAM_PUB)
        user_base_ata  = get_associated_token_address(user,      base_mint, TOKEN_2022_PROGRAM_PUB)
        user_quote_ata = get_associated_token_address(user,      quote_mint, TOKEN_2022_PROGRAM_PUB)
    ...
    async def deposit(
        ...
        if debug_prints:
            print("DEBUG deposit() base_in_raw:", base_in_raw)
            print("DEBUG deposit() quote_needed_lamports:", quote_needed_lamports)
            print("DEBUG deposit() max_base_in:", max_base_in)
            print("DEBUG deposit() max_quote_in:", max_quote_in)

        # LP supply (raw)
        lp_supply_raw = int(pool_data.get("lp_supply") or 0)
        if lp_supply_raw <= 0:
            if debug_prints:
                print("LP supply is zero – cannot estimate LP out.")
            return False
        
        if debug_prints:
            rpc_supply = int((await self.async_client.get_token_supply(lp_mint)).value.amount)
            print("DEBUG lp_supply pool_state:", lp_supply_raw)
            print("DEBUG lp_supply rpc_mint  :", rpc_supply)

        # Target LP to mint for *this* base deposit (raw)
        lp_target_raw = max(base_in_raw * lp_supply_raw // base_res_raw, 1)

        # Max inputs (slippage applies here, not to LP target)
        max_base_in  = int(base_in_raw * (1 + slippage_pct / 100))
        max_quote_in = int(quote_needed_lamports * (1 + slippage_pct / 100))

        if debug_prints:
            print("DEBUG lp_supply_raw:", lp_supply_raw)
            print("DEBUG lp_target_raw:", lp_target_raw)
            print("DEBUG max_base_in:", max_base_in)
            print("DEBUG max_quote_in:", max_quote_in)
        ...
        # SOL balance (needed if quote is WSOL and we wrap)
        sol_balance = (await self.async_client.get_balance(user)).value

        # Be conservative: assume we might need rent buffer for a newly created WSOL ATA.
        needed = quote_needed_lamports + int(0.002 * LAMPORTS_PER_SOL) + int(0.003 * LAMPORTS_PER_SOL)

        if sol_balance < needed:
            if debug_prints:
                print("Not enough SOL to wrap. SOL_BALANCE < NEEDED triggered")
                print("SOL balance:", sol_balance, "needed:", needed)
            return False
        ...
        # Detect which token program each mint uses (legacy vs Token-2022)
        base_token_program  = await self._token_program_for_mint(base_mint)
        quote_token_program = await self._token_program_for_mint(quote_mint)
        lp_token_program    = await self._token_program_for_mint(lp_mint)

        created_quote_ata = False  # track whether we created it (for safe close later)

        # Create ATAs as needed (with the correct token program for each mint)
        wsol_ix = await self._create_ata_if_needed_for_owner(user, user, quote_mint, quote_token_program)
        if wsol_ix:
            ix.append(wsol_ix)
            created_quote_ata = True

        base_ix = await self._create_ata_if_needed_for_owner(user, user, base_mint, base_token_program)
        if base_ix:
            ix.append(base_ix)

        lp_ix = await self._create_ata_if_needed_for_owner(user, user, lp_mint, lp_token_program)
        if lp_ix:
            ix.append(lp_ix)

        # Derive ATAs using the correct token program for each mint
        user_base_ata  = get_associated_token_address(user, base_mint, base_token_program)
        user_quote_ata = get_associated_token_address(user, quote_mint, quote_token_program)
        user_lp_ata    = get_associated_token_address(user, lp_mint, lp_token_program)

        # Only wrap SOL + sync_native when quote is WSOL and uses legacy Tokenkeg program
        if quote_mint == WSOL_MINT and quote_token_program == TOKEN_PROGRAM_PUB:
            wrap_lamports = quote_needed_lamports

            # If we created the WSOL ATA in this tx, add a buffer so rent doesn't steal from the wrapped amount
            if created_quote_ata:
                wrap_lamports += int(0.003 * LAMPORTS_PER_SOL)  # 0.003 SOL buffer (safe)

            ix.append(self._build_system_transfer_ix(user, user_quote_ata, wrap_lamports))

            ix.append(
                sync_native(
                    SyncNativeParams(program_id=TOKEN_PROGRAM_PUB, account=user_quote_ata)
                )
            )
            
        else:
            if debug_prints:
                print("Quote mint is not WSOL/legacy; skipping SOL wrap+sync_native.")
        
        implied_base_in = lp_target_raw * base_res_raw // lp_supply_raw
        print("DEBUG implied_base_in_raw:", implied_base_in, "=> ui:", implied_base_in / (10**dec_base))

        # Build PumpSwap deposit instruction
        ix.append(
            self._build_pumpswap_deposit_ix(
                ...
                pool_base_ata       = Pubkey.from_string(pool_data["pool_base_token_account"]),
                pool_quote_ata      = Pubkey.from_string(pool_data["pool_quote_token_account"]),
                lp_token_amount_out = lp_target_raw,
                max_base_amount_in  = max_base_in,
                max_quote_amount_in = max_quote_in,
            )
        )

        # Close only if we wrapped WSOL into a freshly created ATA in this call
        if quote_mint == WSOL_MINT and quote_token_program == TOKEN_PROGRAM_PUB and created_quote_ata:
            ix.append(
                close_account(
                    CloseAccountParams(
                        program_id=TOKEN_PROGRAM_PUB,
